...
pub fn mutationError(r: zap.Request, err: anyerror, fallback: []const u8) !void {
    switch (err) {
        ...
        error.TaskQuotaExceeded => try jsonError(r, 422, "Task quota reached. Trash and recurring successors count too. Existing data is preserved; ask the operator about capacity."),
        error.TextQuotaExceeded => try jsonError(r, 422, "Task text quota reached. Shorten titles, notes or tags, or ask the operator about capacity. Trash still counts."),
        error.WorkspaceQuotaExceeded => try jsonError(r, 422, "Owned workspace quota reached. Use an existing workspace or contact the operator."),
