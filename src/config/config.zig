...
const ENV_KEYS = [_][]const u8{
    ...
    "OWNED_WORKSPACE_LIMIT",      "REMINDERS_PROCESS_ONCE",
};
...
test "one-shot worker controls are recognized from the process environment" {
    for ([_][]const u8{ "MAIL_PROCESS_ONCE", "REMINDERS_PROCESS_ONCE" }) |required| {
        var found = false;
        for (ENV_KEYS) |key| {
            if (std.mem.eql(u8, key, required)) found = true;
        }
