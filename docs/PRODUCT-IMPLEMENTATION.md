...
| P4 | Pagination, server-side search/filter/sort, resource quotas and load measurements | [D1 pagination](TASK-PAGINATION.md): 2,106 rows; [D2a versions](TASK-VERSIONS.md): conditional writes, two-tab recovery and concurrent-write tests | D1 and D2a complete; server-side filtering/sorting, storage quotas and concurrent capacity measurements remain |
