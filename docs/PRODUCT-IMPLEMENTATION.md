...
| P1 | Atomic password changes/session revocation, account deletion and invite consumption; transaction-bound authorization for role/member/task mutations | [181 passing checks, promotion and limitations](PRODUCT-P1-VERIFICATION.md), including real DB rollback/concurrency cases | Complete; live source 84d1936 |
| P1b | Remove DB administrative credentials from startup arguments; review bootstrap/rotation and local process visibility | Disposable restart with persistent root identity; unrelated local UID cannot read the secret from process arguments; verified recovery before production DB restart | Urgent follow-up |
| P2 | Persistent transactional email outbox; bounded jobs, retry/backoff, claims/leases, expiry, delivery state and operational visibility | Restart does not lose queued jobs; concurrent workers do not claim the same lease; SMTP failure/recovery test; secrets protected and expired links not mailed | Planned |
| P3 | Real task trash/restore, parent-child consistency and scoped undo UI | Restore preserves IDs, notes, dates, assignment, recurrence and relationships; cross-workspace operations denied; keyboard/mobile tests | Planned |
| P4 | Pagination, server-side search/filter/sort, resource quotas and load measurements | More than 2,000 tasks accessible without silent truncation; bounded requests; export remains complete; documented measured capacity | Planned |
