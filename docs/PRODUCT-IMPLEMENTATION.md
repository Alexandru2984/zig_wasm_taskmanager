...
| P6 | Team lifecycle: minimal member directory, assignment picker, rename/archive workspace, owner transfer and per-workspace saved-view sync | [P6a directory/assignment/rename](TEAM-COLLABORATION.md); [P6b ownership](WORKSPACE-OWNERSHIP.md); [P6c archive](WORKSPACE-ARCHIVE.md); [P6d private saved views](SAVED-VIEWS.md): 426 regression + 9 operator-helper checks, restored-copy migration and restricted canary | Complete; saved views are personal, refresh-based and available while archived; account-deletion/access-management rules remain |
